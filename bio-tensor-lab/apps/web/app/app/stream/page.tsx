...
export default function StreamPage() {
  ...
  async function startDemoIngest() {
    setLoading(true);
    try {
      // Send 10 chunks of synthetic ECG-like data
      for (let i = 0; i < 10; i++) {
        const fs = 200;
        const len = 1000;
        ...
        for (let n = 0; n < len; n++) {
          const t = n / fs;
          // rudimentary pseudo-ECG: sum of sines + spike
  ...
  // Flatten chunks to plot a short window (last chunk only for simplicity)
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-10 space-y-6">
      ...
      <p className="text-neutral-600 dark:text-neutral-300">
        Click "Start Demo Ingest" to push synthetic ECG chunks into TimescaleDB. The chart refreshes every 3s.
      </p>
      <div className="flex gap-3">
        ...
        <button onClick={async () => {
          const res = await fetch(`${API}/signals/extract-and-predict`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ patientId, channel }) });
